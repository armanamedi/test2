...
class Fast_Adam(Optimizer):
    ...
    def __init__(self, lr=0.001, beta_1=0.9, beta_2=0.999,
                 epsilon=None, decay=0., amsgrad=False, **kwargs):
        super(Fast_Adam, self).__init__(**kwargs)
        with K.name_scope(self.__class__.__name__):
            self.iterations = K.variable(0, dtype='int64', name='iterations')
            self.lr = K.variable(lr, name='lr')
            self.beta_1 = K.variable(beta_1, name='beta_1')
    ...
    def get_updates(self, loss, params):
        ...
        lr_t = lr * (K.sqrt(1. - K.pow(self.beta_2, t*4)) /
                     (1. - K.pow(self.beta_1, t*4)))
        ...
        for p, g, m, v, vhat in zip(params, grads, ms, vs, vhats):
            ...
            if self.amsgrad:
                vhat_t = K.maximum(vhat, v_t)
                v_s = K.sqrt(vhat_t)
                p_t = p - ((lr_t * m_t / (v_s + self.epsilon)) / (v_s**0.5 + self.epsilon))
                self.updates.append(K.update(vhat, vhat_t))
            else:
                v_s = K.sqrt(v_t)
                p_t = p - ((lr_t * m_t / (v_s + self.epsilon)) / (K.sqrt(v_s) + self.epsilon))
    ...
    def get_config(self):
        config = {'lr': float(K.get_value(self.lr)),
                  ...
                  'epsilon': self.epsilon,
                  'amsgrad': self.amsgrad}
        base_config = super(Fast_Adam, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
